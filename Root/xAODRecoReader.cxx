...
bool xAODRecoReader::processEvent(xAOD::TEvent *xaodEvent,xAOD::TStore */*store*/) {
  ...
  for (const auto& electron : *electrons) {
    ...
    event->addElectron(tlv,electron->charge(),id,idx++);
    delete corrElec;
  }
  idx=0;
  ...
  for (const auto& muon : *muons) {
    ...
    event->addMuon(tlv,muon->charge(),id,idx++);
    delete corrMuon;
  }
...
void xAODRecoReader::processFilesInternal(const std::vector<std::string>& inputFileNames) {
