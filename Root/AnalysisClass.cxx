...
AnalysisObjects operator+(const AnalysisObjects& lhs, const AnalysisObjects& rhs) {
  ...
  for(const auto &cand : lhs) combined.push_back(cand);
  for(const auto &cand : rhs) combined.push_back(cand);
...
AnalysisObjects AnalysisClass::filterObjects(const AnalysisObjects& cands,
  ...
  for(const auto& cand : cands) {
    if ((cand.Pt()>=ptCut) &&
	(fabs(cand.Eta())<etaCut) &&
	(cand.pass(id))) reducedList.push_back(cand);
  }
  return reducedList;
}
...
int AnalysisClass::countObjects(const AnalysisObjects& cands,
		 float ptCut, float etaCut, int id) {
  int count=0;
  for(const auto& cand : cands) {
...
float AnalysisClass::calcAMT2(const AnalysisObject &o1, const AnalysisObject &o2, const AnalysisObject &met, float m1, float m2) {
  return  asymm_mt2_lester_bisect::get_mT2(o1.M(),o1.Px(),o1.Py(),
					   o2.M(),o2.Px(),o2.Py(),
					   met.Px(),met.Py(),m1,m2);
}
...
static TVectorD calcEigenValues(const AnalysisObjects& jets) {
  ...
  for(const auto& jet : jets) {
    ...
    momTensor(1,0) += jet.Py()*jet.Px(); momTensor(1,1) += jet.Py()*jet.Py(); momTensor(1,2) += jet.Py()*jet.Pz();
    momTensor(2,0) += jet.Pz()*jet.Px(); momTensor(2,1) += jet.Pz()*jet.Py(); momTensor(2,2) += jet.Pz()*jet.Pz();
    norm += jet.Vect().Mag2();
  }
  momTensor *= 1./norm;
