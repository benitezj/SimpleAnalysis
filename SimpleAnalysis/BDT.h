...
/*
 *  Copyright 2017, Shunsuke Adachi, Univeristy of Tokyo
 *
...
namespace BDT {
  ...
  class BDTReader {
    ...
    BDTReader(
        ...
        std::vector<std::string> variableDefs,
        std::string xmlfilename1, std::string xmlfilename2="") 
      : m_methodname(methodname), 
      m_variableDefs(variableDefs),
      ...
      m_reader1(0), m_reader2(0), m_reader(0)
      {
        ...
        if(m_xmlfilename2!="") m_useDoubleXML = true;
        m_rand = TRandom3();

        // Initialize reader
        m_reader1 = new TMVA::Reader( "!Color:Silent" );    
        if(m_useDoubleXML) m_reader2 = new TMVA::Reader( "!Color:Silent" );    
        ...
        m_reader1->BookMVA( methodName,PathResolverFindCalibFile("SimpleAnalysis/"+m_xmlfilename1) );
        if(m_useDoubleXML){
          m_reader2->BookMVA( methodName,PathResolverFindCalibFile("SimpleAnalysis/"+m_xmlfilename2) );
        }
      };
    ...
    void setVariables(void){
      for( auto label : m_variableDefs ){
        ...
        if(m_useDoubleXML) m_reader2->AddVariable(var, tmp);
    ...
    // Initililzation for each event (Need to be called before setValue() & getBDT() )
    void initInEventLoop(ULong64_t eventNumber=0){
      m_eventNumber = eventNumber; 
      // Select reader
      if( m_eventNumber==0  ) m_eventNumber = m_rand.Integer(99999999);
      if(m_useDoubleXML){ 
      ...
      }else  m_reader = m_reader1;
      // Clean values in variable containers
      for( auto var : *m_variables       ){ *(var.second) = 0.; }
    };
    ...
    void setValue(std::string label, Float_t value){
      ...
      if( itr==end ){
        std::cout << "  BDT::BDTReader::setValue() : Error!! Cannot find variable label \"" << label << "\"!" << std::endl;
        std::cout << "  -->> Skip set \"" << label <<"\" = " << value << " !!" << std::endl;
        return;
      }else{
        *((*itr).second) = value ;
      }
