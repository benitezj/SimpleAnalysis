...
enum AnalysisObjectType {
    ELECTRON, MUON, TAU, PHOTON, JET, FATJET, MET, COMBINED
};
...
enum AnalysisElectronID {
    EVeryLooseLH = 1 << 0, ELooseLH = 1 << 1, EMediumLH = 1 << 2, ETightLH = 1 << 3, ELooseBLLH = 1 << 4, EIsoGradientLoose = 1 << 8, EIsoBoosted = 1 << 9, // from 3-bjet paper
    EIsoFixedCutTight = 1 << 10, EIsoLooseTrack = 1 << 11, EIsoLoose = 1 << 12, EIsoGradient = 1 << 13, EIsoFixedCutLoose = 1 << 14, EIsoFixedCutTightTrackOnly = 1 << 15, ED0Sigma5 = 1 << 16, EZ05mm = 1 << 17, EGood = EVeryLooseLH | ELooseLH | EMediumLH | ETightLH | ELooseBLLH | ED0Sigma5 | EZ05mm, EIsoGood = EGood | EIsoGradientLoose | EIsoBoosted | EIsoFixedCutTight | EIsoLooseTrack | EIsoLoose | EIsoGradient | EIsoFixedCutLoose | EIsoFixedCutTightTrackOnly
};

enum AnalysisMuonID {
    MuLoose = 1 << 0, MuMedium = 1 << 1, MuTight = 1 << 2, MuVeryLoose = 1 << 3, MuIsoGradientLoose = 1 << 8, MuIsoBoosted = 1 << 9, // from 3-bjet paper
    MuIsoFixedCutTightTrackOnly = 1 << 10, MuIsoLooseTrack = 1 << 11, MuIsoLoose = 1 << 12, MuIsoGradient = 1 << 13, MuIsoFixedCutLoose = 1 << 14, MuD0Sigma3 = 1 << 16, MuZ05mm = 1 << 17, MuNotCosmic = 1 << 18, MuQoPSignificance = 1 << 19, MuCaloTaggedOnly = 1 << 20, MuGood = MuLoose | MuMedium | MuTight | MuVeryLoose | MuD0Sigma3 | MuZ05mm | MuNotCosmic | MuQoPSignificance, MuIsoGood = MuGood | MuIsoGradientLoose | MuIsoBoosted | MuIsoFixedCutTightTrackOnly | MuIsoLooseTrack
};

enum AnalysisTauID {
    TauLoose = 1 << 0, TauMedium = 1 << 1, TauTight = 1 << 2, TauOneProng = 1 << 10, TauThreeProng = 1 << 11, TauGood = TauLoose | TauMedium | TauTight, TauIsoGood = TauGood
};

enum AnalysisPhotonID {
    PhotonLoose = 1 << 0, PhotonTight = 1 << 1, PhotonIsoFixedCutLoose = 1 << 8, PhotonIsoFixedCutTight = 1 << 9, PhotonGood = PhotonLoose | PhotonTight, PhotonIsoGood = PhotonGood | PhotonIsoFixedCutLoose | PhotonIsoFixedCutTight
};

enum AnalysisJetID {
    LooseBadJet = 1 << 8, TightBadJet = 1 << 9, JVT50Jet = 1 << 10, LessThan3Tracks = 1 << 11, // most signal jets should fail this
    GoodJet = LooseBadJet | TightBadJet | JVT50Jet, BTag85MV2c20 = 1 << 0, BTag80MV2c20 = 1 << 1, BTag77MV2c20 = 1 << 2, BTag70MV2c20 = 1 << 3, BTag85MV2c10 = 1 << 4, BTag77MV2c10 = 1 << 5, BTag70MV2c10 = 1 << 6, BTag60MV2c10 = 1 << 7, GoodBJet = BTag85MV2c20 | BTag80MV2c20 | BTag77MV2c20 | BTag70MV2c20 | BTag85MV2c10 | BTag77MV2c10 | BTag70MV2c10 | BTag60MV2c10 | GoodJet, TrueLightJet = 1 << 27, //These should not be used for actual analysis selection, only for understanding
    TrueCJet = 1 << 28, TrueBJet = 1 << 29, TrueTau = 1 << 30,

};

enum AnalysisFatJetID {
    LooseFatJet = 1 << 8, GoodFatJet = LooseFatJet
};

class AnalysisObject: public TLorentzVector {
    public:
        AnalysisObject(double Px, double Py, double Pz, double E, int charge, int id, AnalysisObjectType type, int orgIndex) :
        ...
        ;
        AnalysisObject(TLorentzVector tlv, int charge, int id, AnalysisObjectType type, int orgIndex) :
                    TLorentzVector(tlv),
                    _charge(charge),
                    _id(id),
                    _type(type),
...
class AnalysisClass {
    public:
        AnalysisClass(const std::string &name) :
                    _name(name),
                    _output(0) {
        ...
        ;
        virtual void setOutput(OutputHandler *output) {
            _output = output;
        }
        ;
        ...
        ;
        virtual void addHistogram(const std::string &label, int bins, float min, float max) {
        ...
        virtual void addHistogram(const std::string &label, std::vector<float> &edges) {
            _output->addHistogram(label, edges);
        }
        virtual void addHistogram(const std::string &label, int binsX, float minX, float maxX, int binsY, float minY, float maxY) {
        ...
        static int countObjects(const AnalysisObjects& cands, float ptCut, float etaCut = 100., int id = 0);

        static float sumObjectsPt(const AnalysisObjects& cands, unsigned int maxNum = 10000, float ptCut = 0);

        static float sumObjectsM(const AnalysisObjects& cands, unsigned int maxNum = 10000, float mCut = 0);
